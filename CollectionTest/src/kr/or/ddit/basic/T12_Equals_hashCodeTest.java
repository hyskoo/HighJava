...
/*
	...
	객체가 서로 같은 지를 비교하기 위해 equals()메소드와 hashCode()메소드를 호출한다.
	그래서 객체가 서로 같은지 여부를 결정하려면 두 메소드를 재정의 해야한다.
	HashSet, HashMap, HashTable에서는 객체가 같은지 여부는 데이터를 추가할 때 검사한다.
	
		- equals()메소드는 두 객체의 내용(값)이 같은지 비교하는 메소드이고,
		- hashCode()메소드는 두 객체가 같은 객체인지를 비교하는 메소드이다.
		
		- equals()와 hashCode()에 관련된 규칙
		1. 두 객체가 같으면 반드시 같은 hashCode값을 가져야 한다.
		2. 두 객체가 같으면 equals()를 호출 했을때 true를 반환해야한다.
			즉, 객체 a, b가 같다면 a.equals(b)와 b.equals(a) 둘다 true이다.
		3. 두 객체의 hashCode가 같다고해서 두 객체가 반드시 같은 객체는 아니다.
		...
		5. hashCode()는 기본적으로 Heap에 있는 각 객체에 대한 메모리 주소 매핑정보를 기반으로 한 정수값을 반환한다.
			...
			- hashCode()에서 사용하는 '해싱 알고리즘'에서 서로 다른 객체에 대한 같은 hashCode값을 만들어 낼 수 있다.
...
//	Object의 hashCode와 equals를 override하여 재정의 해준다.
	@Override
	public int hashCode() {
		/*final int prime = 31;
		int result = 1;
