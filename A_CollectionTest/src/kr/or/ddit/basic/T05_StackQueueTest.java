package kr.or.ddit.basic;

import java.util.LinkedList;

public class T05_StackQueueTest {
	/*
	  Stack = 후입선출(LIFO) 의 자료구조
	  Queue = 선입선출(FIFO) 의 자료구조
	  
	  Stack과 Queue는 LinkedList를 이용하여 사용할 수 있다.
	  
	 */	
	public static void main(String[] args) {
		LinkedList<String> stack = new LinkedList<>();
		
		/*
			Stack의 명령
			1) 자료 입력 : push(저장할 값)
			2) 자료 출력 : pop() => 자료를 꺼내온 후 꺼내온 자료는 stack에서 삭제된다
		 */
		stack.push("홍길동");
		stack.push("일지메");
		stack.push("변학도");
		stack.push("강감찬");
		System.out.println("현재의 stack값들 : " + stack );
		
		String data = stack.pop();
		System.out.println("꺼내온 자료 : " + data);
		System.out.println("꺼내온 자료 : " + stack.pop());
		System.out.println("현재의 stack값들 : " + stack );
		
		stack.push("성춘향");
		System.out.println("현재의 stack값들 : " + stack );
		System.out.println("꺼내온 자료 : " + stack.pop());
		
		System.out.println("======================================");
		System.out.println();
		
	/*
		Queue의 명령
		
		1) 자료 입력 : ofter(저장할 값)
		2) 자료 출력 : poll() => 자료형 Queue에서 꺼내온 후 꺼낸 자료는 Queue에서 삭제된다
	 */
		LinkedList<String> queue = new LinkedList<>();
		
		queue.offer("홍길동");
		queue.offer("일지메");
		queue.offer("변학도");
		queue.offer("강감찬");
		
		System.out.println("현재의 Queue값들 : " + queue );
		
		String temp = queue.poll();
		System.out.println("꺼내온 자료 : " + temp);
		System.out.println("꺼내온 자료 : " + queue.poll());
		System.out.println("현재의 Queue값들 : " + queue );
		
		if (queue.offer("성춘향")) {
			System.out.println("신규 등록 자료 : 성춘향");
		}
		System.out.println("현재의 Queue값들 : " + queue );
		System.out.println("꺼내온 자료 : " + queue.poll());
		System.out.println("현재의 Queue값들 : " + queue );
	}
}

