...
/*
   ...
   형식) new InputStreamReader(바이트기반스트림객체, 인코딩방식)
    ...
    한글 인코딩 방식은 크게 UTF-8과 EUC-KR 방식으로 나누어 볼수 있다.
    원래 한글윈도우는 CP949방식을 사용했는데, 윈도우를 개발한 마이크로소프트에서 EUC-KR방식
    에서 확장하였기 때문에 MS949라고도 부른다.
    한글 윈도우의 메모장에서 이야기하는 ANSI 인코딩이란, CP949(Code Page 949)를말함.
  CP949는 EUC-KR의 확장이며, 하위호환성이 있다.
  - MS949 => 윈도우의 기본 한글 인코딩 방식(ANSI계열)
  - UTF-8 => 유니코드 UTF-8 인코딩방식(영문자및 숫자:1byte,한글:3bytes):가변적
  ...
  ASCII => extended ASCII(ISO 8859-1) 
        => 조합형(초성,중성,종성),완성성(KSC 5601)
                      =>윈도우계열:CP949(확장완성형)
                      =>유닉스계열:EUC-KR(확장유닉스코드)
  
  * 유니코드(Unicode)
  서로 다른 문자 인코딩을 사용하는 컴퓨터간의 문서교환에 어려움을 겪게 되고, 이런 문제점을 
  해결하기 위해 전 세계의 모든문자를 하나의 통일된 문자집합(Charset)으로 표현함.
  처음엔 2byte(65536)로 표현했지만, 부족해져 21bit(약200만문자)로 확장되었음.
