...
public class T01_FileTest {
	public static void main(String[] args) throws IOException {
		...
		/*
			디렉토리(폴더)만들기
			1. mkdir()  => File객체의 경로중 마지막위치의 디렉토리를 만든다.
						=> 중간의 경로가 모두 미리 만들어져 있어야 한다
			
			2. mkdirs() => 중간의 경로가 없으면 중간의 경로도 새롭게 만든 후 마지막 위치의 디렉토리를 만들어 준다.
			
			=> 위 1,2 메소드 모두 만들기를 성공하면 [true], 실패하면 [false]를 반환
		 */
		File file5 = new File("d:/D_Other/연습용");
		if (file5.mkdir()) {
			System.out.println(file5.getName() + " 만들기 성공!");
		} else {
			System.out.println(file5.getName() + " 만들기 실패ㅠㅠ");
		}
		System.out.println();
		...
		if (file6.mkdirs()) {
			...
			System.out.println(file6.getName() + " 만들기 실패ㅠㅠ");
		}
	}
}
